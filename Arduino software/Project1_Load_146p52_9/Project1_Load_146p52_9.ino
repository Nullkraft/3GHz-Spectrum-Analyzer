...
/************************************************
 *  Name    : Project1_Load_146p52_9.ino
 *  Author  : WN2A
 *  Date    : 15 June 2021
 *  Version : 3.0
...
// RF = 146.52 MHz
// LO1 = 3776.52MHz
//        LO1      R0    R1    R2    R3    R4    R5    R6    R7    R8    R9    R10   R11   R12   R13   R14
...
// 270 MHz Opt Fract Opt; Dig Lockdet MuxOut Fpfd=5 MHz
...
// int Byte30[] = {0x00, 0x11, 0x42, 0x03, 0x1C, 0x05, 0x42};
// R2=0x58011242 Dig Lockdet  or R6=0x40011242 TriState


// 270 MHz Opt Fract Opt; Dig Lockdet MuxOut Fpfd=15 MHz
//      LO3      R0    R1    R2    R3    R4    R5    R6
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("Project1_Load_146p52_9.ino for Project#1 Spectrum Analyzer RF=146.52MHz");
  ...
  for (int y = 2; y >= 0; y--) 
  {
    ...
    for (int x = 13; x >= 0; x--) {
      ShiftLO1(x);                    // Program LO1=3776.52 MHz with LD on Mux
      delay (pauseMillis);
    }
...
void writeChip(int LD_PIN, int DATA_PIN, int CLK_PIN, int bitOrder, int DATA)
{
  Serial.println();
  Serial.print("writeChip in bit order = ");
  Serial.println(bitOrder);
  digitalWrite(LD_PIN, LOW);
  shiftOut(DATA_PIN, CLK_PIN, bitOrder, DATA);
  delayMicroseconds(1);
